    ...
    for epoch in tqdm(range(epochs)):
        ...
        for batch_samples in dataloader:
            for _ in range(aug_multiple):
                ...
                # 添加这行代码来修复0入度节点问题
                import dgl
                if isinstance(aug_gs, list):
                    aug_gs = [dgl.add_self_loop(g) for g in aug_gs]
                else:
                    aug_gs = dgl.add_self_loop(aug_gs)
        ...
        epoch_time = time.time() - epoch_start  # 计算当前 epoch 运行时间
        if epoch % 50 == 0:
            print(f'epoch {epoch} loss: ', epoch_loss)
        scheduler.step(np.mean(running_loss))

    total_time = time.time() - start_time  # 计算训练总时间
    average_time = total_time / epochs  # 计算每个 epoch 的平均时间

    print(f'Training completed in {total_time:.8f} seconds ({average_time:.8f} seconds per epoch)')
    return model
# # Model Training
# def train(samples, config):
#     # hyperparameters
