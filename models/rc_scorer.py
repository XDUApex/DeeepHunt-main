...
def naive_scorer(cases, samples, loss_df, node_hash, window_size=10, pre=0.0, suc=-0.0):
    ...
    for case_id, case in cases.iterrows():
        ...
        end_ts = case['timestamp'] + (window_size*60/2)
        series_res = loss_df[(loss_df['timestamp']>=start_ts)&(loss_df['timestamp']<end_ts)].set_index('timestamp').mean()
        filtered_samples = [sample for sample in samples if sample[0] >= start_ts and sample[0] < end_ts]
        if filtered_samples:
            g = filtered_samples[-1][1]
        else:
            g = samples[0][1]

        # 用节点名作为 key
        score_map = {node: series_res[node] for node in node_hash if node in series_res}
        final_score_map = {}
        for node in score_map:
            idx = node_hash[node]
            pre_nodes = [n for n in g.successors(idx).tolist() if n != idx]
            pre_names = [k for k, v in node_hash.items() if v in pre_nodes]
            pre_score = np.max([score_map.get(pn, 0) for pn in pre_names]) if pre_names else 0

            suc_nodes = [n for n in g.predecessors(idx).tolist() if n != idx]
            suc_names = [k for k, v in node_hash.items() if v in suc_nodes]
            suc_score = np.max([score_map.get(sn, 0) for sn in suc_names]) if suc_names else 0

            final_score_map[node] = score_map[node] + pre * pre_score + suc * suc_score

        ranks = sorted(final_score_map.items(), key=lambda x: x[1], reverse=True)
        for i in range(len(ranks)):
            test_df.loc[case_id, f'Top{i+1}'] = '%s:%s' % ranks[i]
    return test_df
