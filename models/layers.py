...
class GraphSAGEEncoder(nn.Module):
    def __init__(self, in_feats, hidden_feats, out_feats, dropout, num_layers, norm):
        ...
        self.input_conv = dgl.nn.GraphConv(in_feats, hidden_feats, norm=norm, allow_zero_in_degree=True)
        self.convs = []
        for _ in range(num_layers - 2):
            self.convs.append(dgl.nn.GraphConv(hidden_feats, hidden_feats, norm=norm, allow_zero_in_degree=True))
        if num_layers > 1:
            self.convs.append(dgl.nn.GraphConv(hidden_feats, out_feats, norm=norm, allow_zero_in_degree=True))
...
class GraphSAGEDecoder(nn.Module):
    def __init__(self, in_feats, hidden_feats, out_feats, dropout, num_layers, norm):
        ...
        self.dropout = nn.Dropout(dropout)
        hidden_feats = hidden_feats if num_layers > 1 else out_feats
        self.input_conv = dgl.nn.GraphConv(in_feats, hidden_feats, allow_zero_in_degree=True)
        self.convs = []
        for _ in range(num_layers - 2):
            self.convs.append(dgl.nn.GraphConv(hidden_feats, hidden_feats, allow_zero_in_degree=True))
        if num_layers > 1:
            self.convs.append(dgl.nn.GraphConv(hidden_feats, out_feats, allow_zero_in_degree=True))
